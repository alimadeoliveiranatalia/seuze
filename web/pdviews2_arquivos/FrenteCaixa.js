...
function ExcluirLinha(Linha)
	{
		if(confirm('VocÍ tem certeza que deseja cancelar este item?'))
			{
				var nro;
				if(Linha == 'Limpar')
					{
...
function ConfirmarPedido(Imprimir)
	{
		...
		if($("#tipo_pagamento:checked").val() == 1 && DesFormataMoeda($("#valor_recebido").val()) < 0.01)
			...
			{
				if(confirm('VocÍ confirma o fechamento deste pedido?'))
					{
...
function Cancelar()
	{
		if($("#CaixaLivre").attr("class") != "caixaLivre" && $("#CaixaLivre").attr("class") != "")
			{
				if(confirm('VocÍ tem certeza que deseja cancelar este pedido?'))
					{
						$(".item").each(function() {
							$(this).remove();
						});
...
function ObterPedido(Pedido)
	{
		if(DesFormataMoeda($("#valor_total_produto").val()) > 0)
			{
				if(confirm('VocÍ tem certeza que n„o deseja salvar este item?'))
					{
...
function FecharJanela()
	{
		if(confirm('VocÍ tem certeza que deseja fechar o PDV?'))
			{
				window.close();
			}
	}
function retirarAcento(objResp)
	{  
		...
		var stringAcentos = new String('‡‚ÍÙ˚„ı·ÈÌÛ˙Á¸¿¬ ‘€√’¡…Õ”⁄«‹');  
